...
async function loadDashboard() {
  try {
    ...
    renderTrend(buildTrendRowsFromDashboard());
  } catch (e) {
...
function buildTrendRowsFromDashboard() {
  if (!dashboardData || !Array.isArray(dashboardData.platforms)) return [];
  return dashboardData.platforms.flatMap(p => {
    if (!Array.isArray(p.trend7d)) return [];
    return p.trend7d.map(row => ({
      code: p.code,
      name: p.name,
      balance: row.balance,
...
async function loadTrend(range) {
  ...
  try {
    ...
    const fallbackRows = currentRange === '7d' ? buildTrendRowsFromDashboard() : [];
    renderTrend(rows && rows.length ? rows : fallbackRows);
  } catch (e) {
