...
async function applySent(platformId, level, redRepeatHours) {
  ...
  if (level === 'yellow') {
    await pool.query(
      "UPDATE alert_state SET current_level='yellow', yellow_sent=1, yellow_sent_at=?, last_eval_at=? WHERE platform_id=?",
      [now, now, platformId]
    );
  } else if (level === 'red') {
    await pool.query(
      "UPDATE alert_state SET current_level='red', red_last_sent_at=?, red_sent_count=red_sent_count+1, last_eval_at=? WHERE platform_id=?",
      [now, now, platformId]
    );
  }
}
...
// 状态机更新：跳过（只更新 level 和 last_eval_at）
async function applySkip(platformId, newLevel) {
  const now = new Date();
  await pool.query(
    "UPDATE alert_state SET current_level=?, last_eval_at=? WHERE platform_id=?",
